...
def main() -> None:
    ...
    Difference with original EuroEval:
      - EuroEval concatenates makes smaller train/test/dev splits
      - It only concatenates part of the dev set that was not in the new dev split, to the test set, 
      and from that concatenation, it subsampels the final test set. Not ideal -> data leakage.
    """
    ...
    train_df = train_df_with_answer.reset_index(drop=True)
    val_df = val_df_with_answer.reset_index(drop=True)
    test_df = test_df_with_answer.reset_index(drop=True)
    ...
    dataset_id = "BramVanroy/squad-nl-v2-full"
